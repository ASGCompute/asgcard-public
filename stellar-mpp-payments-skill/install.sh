...
REPO="${STELLAR_MPP_SKILL_REPO:-https://github.com/ASGCompute/asgcard-public.git}"
SOURCE_ROOT="stellar-mpp-payments-skill"
...
if [ -d "$HOME/.claude" ] || command -v claude >/dev/null 2>&1; then
  ...
  rm -rf "$DEST"
  cp -r "$TEMP_DIR/repo/$SOURCE_ROOT/$SKILL_DIR" "$DEST"
...
if [ -f ".cursorrules" ] || command -v cursor >/dev/null 2>&1 || [ -d ".cursor" ]; then
  cp "$TEMP_DIR/repo/$SOURCE_ROOT/.cursorrules" ./.cursorrules
  mkdir -p ".claude/skills"
  rm -rf ".claude/skills/$SKILL_DIR"
  cp -r "$TEMP_DIR/repo/$SOURCE_ROOT/$SKILL_DIR" ".claude/skills/$SKILL_DIR"
  echo "Installed .cursorrules and .claude/skills/$SKILL_DIR to the current project"
  installed=true
fi
